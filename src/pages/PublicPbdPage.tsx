...
export function PublicPbdPage() {
  ...
  const [cohort,setCohort]=useState<CohortSummary>({candidates:0,mbpk:0}); const [subjectCohorts,setSubjectCohorts]=useState<SubjectCohort[]>([]); const [error,setError]=useState('');
  ...
  async function load(){
    ...
    const [summaryResult,cohortResult,subjectCohortResult]=await Promise.all([
      ...
      supabase.rpc('v2_public_subject_cohorts',{p_year:year,p_class_name:classArg,p_mode:'PBD'}),
    ]);
    if(summaryResult.error||cohortResult.error||subjectCohortResult.error){setError(summaryResult.error?.message||cohortResult.error?.message||subjectCohortResult.error?.message||'Ralat memuatkan data.');setRows([]);setCohort({candidates:0,mbpk:0});setSubjectCohorts([]);return;}
  ...
  const selectedCandidates=selectedCohort?.candidates||0;
  const selectedMtmPct=selectedCandidates?mtmCount/selectedCandidates*100:0;
  const selectedInterventionPct=selectedCandidates?interventionCount/selectedCandidates*100:0;
  ...
  return <div className="public-report"><PageHeader title="Pelaporan PBD" />
    ...
    <div className="stats-grid four">
      <StatCard icon={Users} label="Bilangan Calon" value={selectedRow?selectedCandidates:cohort.candidates}/>
      <StatCard icon={Accessibility} label="MBPK" value={selectedRow?Number(selectedCohort?.mbpk||0):cohort.mbpk}/>
      <StatCard icon={Target} label={selectedRow?`Jumlah MTM — ${selectedRow.subject_name}`:'Jumlah MTM'} value={selectedRow?`${mtmCount} (${selectedMtmPct.toFixed(1)}%)`:'—'} hint={selectedRow?undefined:'Pilih mata pelajaran'} tone="amber"/>
      <StatCard icon={AlertTriangle} label={selectedRow?`Jumlah Intervensi — ${selectedRow.subject_name}`:'Jumlah Intervensi'} value={selectedRow?`${interventionCount} (${selectedInterventionPct.toFixed(1)}%)`:'—'} hint={selectedRow?'TP1–TP2':'Pilih mata pelajaran'} tone="red"/>
    </div>
    <div className="subject-analysis-list">{visibleRows.map(r=>{
      ...
      const sc=subjectCohorts.find(c=>c.subject_code===r.subject_code); const candidates=Number(sc?.candidates||0); const mtmPct=candidates?subjectMtm/candidates*100:0; const interventionPct=candidates?subjectIntervention/candidates*100:0;
      return <GlassCard className="subject-card" key={r.subject_code}><div className="subject-card-head"><div><div className="eyebrow">{assessment}</div><h2>{r.subject_name}</h2></div><div className="metric-pair"><span>Bilangan Calon<strong>{candidates}</strong></span><span>Direkod<strong>{Number(r.total||0)}</strong></span><span>MTM<strong>{subjectMtm} ({mtmPct.toFixed(1)}%)</strong></span><span>Intervensi<strong>{subjectIntervention} ({interventionPct.toFixed(1)}%)</strong></span></div></div><div className="subject-card-body single"><DistributionBars items={items}/></div></GlassCard>;
    })}</div>
  </div>;
}
